...
final class ScanBenchmarkTests: XCTestCase {
    func testRealWorldScanBenchmark() async throws {
        ...
        guard environment["PATHLIGHT_BENCH"] == "1" else {
            throw XCTSkip("Set PATHLIGHT_BENCH=1 to run the real-world scan benchmark.")
        }

        let benchmarkPath = environment["PATHLIGHT_BENCH_PATH"] ?? "/Applications"
        ...
        print(
            """
            PATHLIGHT_BENCH_RESULT path=\(targetURL.path)
    ...
    func testWideDirectoryClassificationBenchmark() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard environment["PATHLIGHT_BENCH_WIDE_DIRECTORY"] == "1" else {
            throw XCTSkip("Set PATHLIGHT_BENCH_WIDE_DIRECTORY=1 to run the wide-directory benchmark.")
        }

        let fileCounts = Self.integerList(
            from: environment["PATHLIGHT_BENCH_WIDE_FILE_COUNTS"],
            defaultValues: [128, 1_000, 10_000]
        )
        let iterations = environment["PATHLIGHT_BENCH_WIDE_ITERATIONS"]
            .flatMap(Int.init)
            .map { max(1, $0) } ?? 3
        let traversalWorkers = environment["PATHLIGHT_BENCH_WIDE_TRAVERSAL_WORKERS"]
            .flatMap(Int.init)
            .map { max(1, $0) } ?? 4
        let classificationWorkers = environment["PATHLIGHT_BENCH_WIDE_CLASSIFICATION_WORKERS"]
            .flatMap(Int.init)
            .map { max(1, $0) } ?? 4
        ...
        for fileCount in fileCounts {
            ...
            for configuration in configurations {
                ...
                print(
                    """
                    PATHLIGHT_BENCH_WIDE_SUMMARY files=\(fileCount)
    ...
    func testFanoutWideDirectoryClassificationBenchmark() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard environment["PATHLIGHT_BENCH_WIDE_FANOUT"] == "1" else {
            throw XCTSkip("Set PATHLIGHT_BENCH_WIDE_FANOUT=1 to run the fanout wide-directory benchmark.")
        }

        let childDirectoryCounts = Self.integerList(
            from: environment["PATHLIGHT_BENCH_WIDE_FANOUT_DIR_COUNTS"],
            defaultValues: [8]
        )
        let filesPerDirectoryCounts = Self.integerList(
            from: environment["PATHLIGHT_BENCH_WIDE_FANOUT_FILES_PER_DIR"],
            defaultValues: [1_000]
        )
        let iterations = environment["PATHLIGHT_BENCH_WIDE_ITERATIONS"]
            .flatMap(Int.init)
        ...
        for childDirectoryCount in childDirectoryCounts {
            for filesPerDirectory in filesPerDirectoryCounts {
                ...
                for configuration in configurations {
                    ...
                    print(
                        """
                        PATHLIGHT_BENCH_WIDE_FANOUT_SUMMARY child_dirs=\(childDirectoryCount)
    ...
    private func makeWideBenchmarkDirectory(fileCount: Int) throws -> URL {
        let rootURL = FileManager.default.temporaryDirectory
            .appending(path: "pathlight-wide-directory-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    private func makeFanoutWideBenchmarkDirectory(
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appending(path: "pathlight-wide-fanout-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
    ...
    private func runWideDirectoryBenchmark(
        ...
        print(
            """
            PATHLIGHT_BENCH_WIDE_RESULT phase=\(phase)
            files=\(fileCount)
            config=\(configuration.name)
            iteration=\(iteration)
            traversal_workers=\(configuration.traversalWorkerDescription)
