...
final class ScanArchiveBenchmarkTests: XCTestCase {
    ...
    func testArchiveExportImportBenchmark() async throws {
        ...
        guard environment["PATHLIGHT_BENCH_ARCHIVE"] == "1" else {
            throw XCTSkip("Set PATHLIGHT_BENCH_ARCHIVE=1 to run archive export/import benchmarks.")
        }

        let iterations = Self.integer(from: environment["PATHLIGHT_BENCH_ARCHIVE_ITERATIONS"], defaultValue: 3)
    ...
    func testArchiveCostProfileBenchmark() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard environment["PATHLIGHT_BENCH_ARCHIVE_PROFILE"] == "1" else {
            throw XCTSkip("Set PATHLIGHT_BENCH_ARCHIVE_PROFILE=1 to run archive cost profile benchmarks.")
        }

        let service = ScanArchiveService()
        let benchmarkCase = Self.benchmarkCases(environment: environment)
            .first { $0.name == (environment["PATHLIGHT_BENCH_ARCHIVE_PROFILE_CASE"] ?? "large") } ??
        ...
        print(
            """
            PATHLIGHT_ARCHIVE_PROFILE_RESULT case=\(benchmarkCase.name) nodes=\(snapshot.treeStore.nodeCount) \
    ...
    private static func wideCase(environment: [String: String]) -> BenchmarkCase {
        let fileCount = integer(from: environment["PATHLIGHT_BENCH_ARCHIVE_WIDE_FILES"], defaultValue: 10_000)
    ...
    private static func deepCase(environment: [String: String]) -> BenchmarkCase {
        let depth = integer(from: environment["PATHLIGHT_BENCH_ARCHIVE_DEEP_DEPTH"], defaultValue: 12_000)
    ...
    private static func largeCase(environment: [String: String]) -> BenchmarkCase {
        let directoryCount = integer(from: environment["PATHLIGHT_BENCH_ARCHIVE_LARGE_DIRS"], defaultValue: 64)
        let filesPerDirectory = integer(from: environment["PATHLIGHT_BENCH_ARCHIVE_LARGE_FILES_PER_DIR"], defaultValue: 1_000)
    ...
    private func makeTemporaryArchiveURL(caseName: String, iteration: Int) throws -> URL {
        let directoryURL = FileManager.default.temporaryDirectory
            .appending(path: "pathlight-archive-bench-\(caseName)-\(iteration)-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        addTeardownBlock {
            try? FileManager.default.removeItem(at: directoryURL)
        }
        return directoryURL.appending(path: "Export.pathlightscan", directoryHint: .isDirectory)
    }
    ...
    private static func resultLine(
        ...
        let sections = sectionSizes
            ...
            .joined(separator: " ")
        return """
        PATHLIGHT_ARCHIVE_BENCH_RESULT case=\(benchmarkCase.name) detail=\(benchmarkCase.detail) iteration=\(iteration) \
