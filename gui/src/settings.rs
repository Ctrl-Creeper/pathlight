...
/// The two settings a person confirms before a stopped folder begins
/// monitoring. They are install-wide on this host, just like the full settings
/// pane and the CLI, so every open watch receives the same recording rules.
...
impl StartDraft {
    pub fn read(storage: &Storage) -> Self {
        Self {
            least_kb: (storage.options().minimum_recorded_byte_delta / BYTES_PER_KB).to_string(),
            latency_ms: storage.latency_ms().to_string(),
            message: None,
        }
    }

    pub fn ui(&mut self, ui: &mut egui::Ui, storage: &Storage) -> Option<StartVerdict> {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Start monitoring").clicked() {
                match self.save(storage) {
                    Ok(()) => verdict = Some(StartVerdict::Start),
    ...
    fn save(&self, storage: &Storage) -> Result<(), String> {
...
impl Draft {
    ...
    pub fn read(storage: &Storage) -> Self {
        ...
        Self {
            ...
            least_kb: (options.minimum_recorded_byte_delta / BYTES_PER_KB).to_string(),
            latency_ms: storage.latency_ms().to_string(),
