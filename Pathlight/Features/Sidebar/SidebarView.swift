...
private struct DiscardPileSidebarButton: View {
    ...
    private var subtitle: String {
        ...
        return "\(summary.itemCount.formatted()) \(summary.itemCount == 1 ? "item" : "items") • \(PathlightFormatters.size(summary.totalAllocatedSize))"
    }
}
...
private struct SidebarTargetRow: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button("Reveal in Finder", systemImage: PathlightSystemImages.revealInFinder) {
                revealInFinder()
            }
