...
struct OnboardingView: View {
    ...
    private var intro: some View {
        VStack(spacing: 12) {
            ...
            Text("Welcome to Pathlight")
                .font(.title2.weight(.semibold))

            Text("Scan folders and disks to see where space is going. Pathlight stays read-only until you choose a file action.")
...
private struct FullDiskAccessSetupCard: View {
    ...
    private var statusMessage: String {
        switch status {
        case .granted:
            return "Pathlight can currently read protected locations. You can start a complete scan now."
        case .notGranted:
            return "Pathlight does not currently have Full Disk Access. Normal scans still work; protected folders may be skipped."
        case .unknown:
            return "Pathlight could not verify Full Disk Access on this Mac. You can still scan ordinary folders."
        }
    }
