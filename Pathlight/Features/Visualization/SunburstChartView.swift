...
struct SunburstChartView: View {
    ...
    private var hoverSummary: ChartSummary? {
        ...
        return ChartSummary(
            ...
            value: PathlightFormatters.size(hoveredSegment.totalSize),
    ...
    private var accessibilityValue: String {
        ...
        return "\(node.name), \(PathlightFormatters.size(node.allocatedSize)), \(summaryStatus(for: node))"
    }
    ...
    private func summary(for node: FileNodeRecord) -> ChartSummary {
        if SunburstFreeSpaceVisualization.isFreeSpaceNodeID(node.id) {
            return ChartSummary(
                ...
                title: node.name,
                value: PathlightFormatters.size(node.allocatedSize),
        ...
        if node.id != rootNode.id,
           let percentText = PathlightFormatters.percentage(part: node.allocatedSize, total: rootNode.allocatedSize) {
