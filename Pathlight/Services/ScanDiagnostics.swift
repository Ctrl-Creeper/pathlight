...
nonisolated final class ScanDiagnostics: @unchecked Sendable {
    ...
    init(environment: [String: String] = ProcessInfo.processInfo.environment) {
        reportLimit = environment["PATHLIGHT_SCAN_DIAGNOSTICS_LIMIT"]
            .flatMap(Int.init)
            .map { max(1, $0) } ?? 30
        let slowThresholdMilliseconds = environment["PATHLIGHT_SCAN_DIAGNOSTICS_SLOW_MS"]
    ...
    static func makeIfEnabled(environment: [String: String] = ProcessInfo.processInfo.environment) -> ScanDiagnostics? {
        guard environment["PATHLIGHT_SCAN_DIAGNOSTICS"] == "1" else { return nil }
    ...
    func makeReport(targetPath: String, elapsedSeconds: Double) -> String {
        ...
        var lines: [String] = [
            "PATHLIGHT_SCAN_DIAGNOSTICS target=\(targetPath) elapsed=\(Self.format(seconds: elapsedSeconds))s",
            "PATHLIGHT_SCAN_DIAGNOSTICS operations"
        ]
        ...
        lines.append("PATHLIGHT_SCAN_DIAGNOSTICS hot_path_buckets")
        ...
        lines.append("PATHLIGHT_SCAN_DIAGNOSTICS slow_events")
