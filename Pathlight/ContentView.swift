    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Label(
                    "Scanned \(PathlightFormatters.date(preview.finishedAt ?? preview.startedAt))",
                    systemImage: "clock"
                )
                .font(.caption)
                .foregroundStyle(.secondary)
            ...
            HStack(spacing: 8) {
                ImportSnapshotStatCard(
                    title: "Scanned Data",
                    value: PathlightFormatters.size(preview.totalAllocatedSize)
                )
                ImportSnapshotStatCard(
                    title: "Files",
                ...
                ImportSnapshotStatCard(
                    title: "Snapshot Size",
                    value: PathlightFormatters.size(preview.archiveSize)
                )
            }
...
private struct ImportSnapshotDetailsPopover: View {
    ...
    var body: some View {
        Grid(alignment: .leading, horizontalSpacing: 18, verticalSpacing: 8) {
            detailRow("Exported", PathlightFormatters.date(preview.exportedAt))
...
private extension ContentView {
    ...
    var pendingTrashMessage: String {
        ...
        guard nodes.count != 1 else {
            return "Pathlight will ask macOS to move \(nodes[0].url.path) to the Trash."
        }
        ...
        return "Pathlight will ask macOS to move \(nodes.count) selected items to the Trash:\n\(shownPaths)\(remainingText)"
    }
}
