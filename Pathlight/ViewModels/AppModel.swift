...
final class AppModel: ObservableObject {
    ...
    private enum FileActionError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .missingScannedIdentity(let path):
                return "Pathlight could not verify the scanned identity for \(path). Rescan before moving it to Trash."
            case .currentIdentityUnavailable(let path, let reason):
                return "Pathlight could not verify the current identity for \(path): \(reason)"
            case .unsupported:
            ...
            case .packageContentsHidden(let settingEnabled):
                if settingEnabled {
                    return "Pathlight scanned this package before package contents were expanded. Rescan this location to zoom into it."
                }
                return "Pathlight scanned this package as a single item. To zoom into it, turn on “Treat app bundles and packages as folders” in Settings, then rescan this location."
            case .folderRequiredForDrop:
                return "Drop a folder or mounted volume to start a scan."
            case .fullDiskAccessSettingsUnavailable:
                return "Pathlight could not open Full Disk Access settings."
            case .readOnlySnapshot:
    ...
    private var importUnavailableMessage: String {
        ...
        return "Pathlight cannot import a snapshot right now."
    }
    ...
    private func sanitizedFileName(_ name: String) -> String {
        ...
        return sanitizedName.isEmpty ? "Pathlight Scan" : sanitizedName
    }
